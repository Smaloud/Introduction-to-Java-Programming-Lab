...
public class BattleField {
    ...
    public void battle(String tp1, String tp2) {
        if (!this.trainerA.getActivatePokemon().isAlive()) this.trainerA.summon();
        if (!this.trainerB.getActivatePokemon().isAlive()) this.trainerB.summon();
    ...
    public String toString() {
        if (this.checkWin() == 1) {
            return String.format("Turn %d:\nTrainer %s's Pokemon %s: %d/%d\nTrainer %s's Pokemon %s: %d/%d\nWinner: %s\n",
                    turn-1, this.trainerA.getName(), trainerA.getActivatePokemon().getName(), this.trainerA.getActivatePokemon().getHP(), this.trainerA.getActivatePokemon().getMaxHP(),
